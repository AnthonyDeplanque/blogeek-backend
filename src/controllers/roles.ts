...
const removeRoleToUser = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  ...
  try {
    ...
    if (role !== ROLE.ROLE_USER) {
      const roleId = rolesQueries.getOneRoleQueryByName(role).then(([[result]]: [[Roles]]) => { return result }).then((value: any) => value.id)
      await rolesToUsersQueries.removeRoleToUserWithIdsQuery(await roleId, id).then(([result]: any) => {
